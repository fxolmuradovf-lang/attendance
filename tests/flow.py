...
async def main() -> None:
    ...
    check("rows follow DEPARTMENT order, not the alphabet",
          transliterate("Axmadov") in table.rows[1].cells[1].text
          and transliterate("Karimov") in table.rows[2].cells[1].text,
          f"{table.rows[1].cells[1].text!r} / {table.rows[2].cells[1].text!r}")
    check("surname sits above the rest of the name",
          table.rows[2].cells[1].text.split("\n")[0].strip() == transliterate("Karimov"),
    ...
    check("departure date and reason in one cell",
          table.rows[2].cells[3].text.startswith(x_day.strftime("%d.%m.%Y"))
          and transliterate("Xizmat safari") in table.rows[2].cells[3].text,
          repr(table.rows[2].cells[3].text))
    check("arrival date present",
          table.rows[2].cells[4].text.strip()
          == (x_day + dt.timedelta(days=3)).strftime("%d.%m.%Y"))
    check("date line matches the input X",
          built.paragraphs[2].text == f"({x_day.strftime('%d.%m.%Y')} й)",
