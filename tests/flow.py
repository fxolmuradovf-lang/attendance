...
async def main() -> None:
    ...
    check("heading date matches the input X, numerically",
          built.paragraphs[2].text == f"({x_day:%d.%m.%Y} й)",
          built.paragraphs[2].text)
    check("heading date is italic",
          all(r.italic for r in built.paragraphs[2].runs))
    check("the table itself carries no numeric dates",
