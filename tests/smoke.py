...
async def main() -> None:
    ...
    check("date line uses DD.MM.YYYY and the 'й' suffix",
          paragraph_text[2] == f"({today.strftime('%d.%m.%Y')} й)")
    ...
    check("surname is on its own line, transliterated",
          first[1].text.split("\n")[0].strip() == transliterate(
              split_name(sorted(doc_rows, key=lambda r: settings.department_index(r["department"]))[0]["full_name"])[0]
          ),
          repr(first[1].text))
    check("lavozim cell is transliterated Cyrillic",
          any(ch in first[2].text for ch in "абвгдеёжзийклмнопрстуфхцчшъэюя"),
          repr(first[2].text))
    check("departure cell holds the date", first[3].text.startswith(
        sorted(doc_rows, key=lambda r: settings.department_index(r["department"]))[0]["start_date"].strftime("%d.%m.%Y")))
    check("departure cell holds the reason in brackets",
          "(" in first[3].text and ")" in first[3].text, repr(first[3].text))
    check("arrival cell is a date",
          len(first[4].text.strip()) == 10 and first[4].text.strip()[2] == ".",
          repr(first[4].text))
