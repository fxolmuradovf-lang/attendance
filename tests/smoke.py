...
async def main() -> None:
    ...
    check("heading date keeps the year for the 'й'",
          paragraph_text[2] == f"({malumot.fmt_day_month(today)} {today.year} й)",
          paragraph_text[2])
    check("no numeric date anywhere in the body",
          not any(f"{today:%d.%m.%Y}" in t for t in paragraph_text))
