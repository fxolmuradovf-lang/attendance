...
async def main() -> None:
    ...
    # --- the (Раис ўринбосари) qualifier is dropped from Лавозими ----------
    check("Latin suffix stripped and spaces tidied",
          malumot.clean_department("Amaliyot departamenti direktori (Rais o'rinbosari)")
          == "Амалиёт департаменти директори")
    check("Cyrillic suffix stripped",
          malumot.clean_department("Амалиёт департаменти директори (Раис ўринбосари)")
          == "Амалиёт департаменти директори")
    check("suffix in the middle leaves no double space",
          malumot.clean_department("Yuridik (Rais o'rinbosari) departamenti direktori")
          == "Юридик департаменти директори")
    check("a dangling comma is trimmed",
          malumot.clean_department("Statistika direktori, (Rais o'rinbosari)")
          == "Статистика директори")
    check("lower-case variant stripped",
          malumot.clean_department("Ichki audit direktori (rais o'rinbosari)")
          == "Ички аудит директори")
    check("surrounding whitespace collapsed",
          malumot.clean_department("Kadrlar direktori   (Rais o'rinbosari)  ")
          == "Кадрлар директори")
    check("a title without the suffix is untouched",
    ...
    # A whole document built from suffixed departments: the column is clean.
    suffixed = [
        dict(rec, department=f"{rec['department']} (Rais o'rinbosari)")
        for rec in doc_rows
    ]
    suffixed_doc = _docx.Document(
        malumot.build_document(
            suffixed, report_date=today,
            department_order=lambda d: settings.department_index(
                d.replace(" (Rais o'rinbosari)", "")
            ),
        )
    )
    lavozim_cells = [r.cells[2].text for r in suffixed_doc.tables[0].rows[1:]]
    check("suffix never reaches the document",
          not any("ўринбосари" in t or "rinbosari" in t for t in lavozim_cells),
          str(lavozim_cells))
    check("no leftover empty brackets or double spaces",
          not any("()" in t or "  " in t or t != t.strip() for t in lavozim_cells),
          str(lavozim_cells))
